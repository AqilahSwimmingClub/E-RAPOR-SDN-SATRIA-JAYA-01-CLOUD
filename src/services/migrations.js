...
const PRESERVED_COLLECTIONS=['students','attendance','manualAttendance','learningObjectives','cpButir','cpButirScores','assessmentScores','cpEvidenceScores','reportScores','subjectMappings','userAccounts','intracurricularActivities','intracurricularScores','dapodikSyncState','dapodikSyncLogs','dapodikMappings','publishedReports'];
...
function stampAppVersion(original,schemaVersion,targetAppVersion){
  const fromAppVersion=String(original.appVersion||'legacy');
  const changed=fromAppVersion!==targetAppVersion;
  if(changed)localStorage.setItem(storageKey(),JSON.stringify({...original,appVersion:targetAppVersion}));
  return {migrated:false,fromSchemaVersion:schemaVersion,toSchemaVersion:schemaVersion,fromAppVersion,toAppVersion:targetAppVersion,appVersionUpdated:changed};
}
