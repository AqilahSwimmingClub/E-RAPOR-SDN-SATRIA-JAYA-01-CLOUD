...
export function transcriptTemplateCsv(session){
  assertTeacher(session);const students=listStudents(session,{classId:session.classId});const subjects=listActiveSubjects(session);const escape=value=>{const text=String(value??'');return /[",\r\n]/.test(text)?`"${text.replace(/"/g,'""')}"`:text;};const rows=students.flatMap(student=>subjects.map(subject=>[student.nisn,student.nis,student.name,subject.id,subject.name,''].map(escape).join(',')));return `\uFEFF${TRANSCRIPT_CSV_HEADERS.join(',')}\r\n${rows.join('\r\n')}${rows.length?'\r\n':''}`;
}

export function transcriptTemplateWorkbook(session){
  assertTeacher(session);const students=listStudents(session,{classId:session.classId});const subjects=listActiveSubjects(session);const rows=[TRANSCRIPT_CSV_HEADERS,...students.flatMap(student=>subjects.map(subject=>[student.nisn,student.nis,student.name,subject.id,subject.name,'']))];return createWorkbookBytes('Nilai Transkrip',rows,{columnWidths:[18,14,28,18,36,12]});
}
