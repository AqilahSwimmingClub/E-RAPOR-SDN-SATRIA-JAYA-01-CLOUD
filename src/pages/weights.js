...
export function renderWeights(session){
  ...
  function drawAll(){
    ...
    allView.innerHTML=`<section class="card weights-bulk-card"><div class="section-head"><div><h3>Bobot & KKTP Seluruh Mata Pelajaran</h3><p>Ubah semua bobot lebih dulu, lalu simpan sekali. Total bobot tiap mapel harus tepat 100%.</p></div><span class="badge badge-active">${baris.length} mapel</span></div><div class="table-scroll"><table class="data-table weights-bulk-table"><thead><tr><th>Mata Pelajaran</th>${ASSESSMENT_TYPES.map(type=>`<th>${escapeHtml(type.label)}</th>`).join('')}<th>Total</th><th>Pakai Bobot</th><th>KKTP</th></tr></thead><tbody>${baris.map(({subject,settings})=>`<tr data-row="${escapeHtml(subject.id)}"><td><strong>${escapeHtml(subject.name)}</strong><span>Kelompok ${escapeHtml(subject.group)}</span></td>${ASSESSMENT_TYPES.map(type=>`<td><input class="input score-input" type="number" min="0" max="100" data-w="${type.id}" value="${Number(settings[type.id])}"/></td>`).join('')}<td><b data-total>0</b></td><td><label class="switch switch-compact"><input type="checkbox" data-use-weights ${settings.useWeights?'checked':''}/></label></td><td><input class="input score-input" type="number" min="0" max="100" data-kktp value="${Number(settings.kktp)}"/></td></tr>`).join('')}</tbody></table></div><div class="actions"><button class="btn btn-primary" data-save-all>${icon('save',17)} Simpan Semua Bobot</button></div></section>`;
    ...
    allView.querySelector('[data-save-all]').onclick=()=>{
      const entri=rows.map(row=>({subjectId:row.dataset.row,kktp:row.querySelector('[data-kktp]').value,
        useWeights:row.querySelector('[data-use-weights]').checked,
...
function draw(){
    ...
    /* Bobotnya tetap dapat disunting saat OFF: angkanya disimpan apa adanya dan langsung
       dipakai lagi begitu toggle dinyalakan kembali. Yang berubah hanya cara menghitung. */
    editor.querySelector('[data-use-weights]').onchange=event=>{event.target.parentElement.lastChild.textContent=event.target.checked?' ON':' OFF';};
  ...
  function formValue(){
    const result={kktp:editor.querySelector('[data-kktp]').value,rubric:rubricValue(),
      useWeights:editor.querySelector('[data-use-weights]').checked};
