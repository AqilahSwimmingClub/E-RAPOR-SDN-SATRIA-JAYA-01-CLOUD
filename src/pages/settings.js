...
export function renderBackupRestore(session){
  ...
  fileButton.onclick=async()=>{
    ...
    try{
      ...
      /* Preview memperlihatkan isi berkas lebih dulu; database baru diubah setelah guru menekan
         Restore dan mengonfirmasi. */
      preview.innerHTML=`<div class="backup-preview"><h3 style="margin-bottom:10px">Preview Backup</h3><div class="preview-grid"><div class="preview-item"><span>Tanggal backup</span><strong>${escapeHtml(new Date(s.exportedAt).toLocaleString('id-ID'))}</strong></div><div class="preview-item"><span>Versi aplikasi</span><strong>${escapeHtml(String(s.appVersion))} · schema ${escapeHtml(String(s.schemaVersion))}</strong></div><div class="preview-item"><span>Kelas</span><strong>${escapeHtml(s.classId)}</strong></div><div class="preview-item"><span>Semester</span><strong>${escapeHtml(s.semester)}</strong></div><div class="preview-item"><span>Tahun Pelajaran</span><strong>${escapeHtml(s.academicYear)}</strong></div><div class="preview-item"><span>Data Siswa</span><strong>${s.students}</strong></div><div class="preview-item"><span>Nilai Penilaian</span><strong>${s.assessmentScores}</strong></div><div class="preview-item"><span>Nilai Rapor</span><strong>${s.reportScores}</strong></div><div class="preview-item"><span>Deskripsi</span><strong>${s.descriptions}</strong></div><div class="preview-item"><span>Absensi</span><strong>${s.attendance}</strong></div><div class="preview-item"><span>Tujuan Pembelajaran</span><strong>${s.objectives}</strong></div><div class="preview-item"><span>Bobot &amp; KKTP</span><strong>${s.weights}</strong></div><div class="preview-item"><span>Mapping</span><strong>${s.mappings}</strong></div><div class="preview-item"><span>Ekstrakurikuler</span><strong>${s.extracurricular}</strong></div><div class="preview-item"><span>Kokurikuler</span><strong>${s.cocurricular}</strong></div><div class="preview-item"><span>Sikap</span><strong>${s.attitudes}</strong></div><div class="preview-item"><span>Catatan Wali Kelas</span><strong>${s.homeroomNotes}</strong></div></div><div class="modal-actions" style="margin-top:12px"><button class="btn btn-light" data-cancel-restore>Batalkan</button><button class="btn btn-danger" data-restore>Restore Data</button></div></div>`;
      preview.querySelector('[data-cancel-restore]').onclick=()=>{parsedPayload=null;preview.innerHTML='';};
