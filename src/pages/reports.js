...
import { getSelectedAssessmentObjectives, listObjectivesForAssessment } from '../services/learning-objectives.js';
...
export function renderReportInput(session,mode='input'){
  ...
  function openDescription(studentId){
    const objectives=listObjectivesForAssessment(session,subjectId,{activeOnly:true});if(!objectives.length){toast('Tambahkan dan aktifkan TP sebelum membuat deskripsi.','warning');return;}
    /* TP acuan yang dicentang guru pada menu Penilaian menjadi sumber utama deskripsi.
       Bila belum ada yang dicentang, cara lama dengan TP terbaik dan TP perlu ditingkatkan
       tetap tersedia sehingga data lama tidak terganggu. */
    const acuan=getSelectedAssessmentObjectives(session,subjectId);
    const acuanRecords=objectives.filter(item=>acuan.includes(item.id));
