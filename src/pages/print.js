...
export function renderPrint(session,initialTab='ledger'){
  ...
  function bindCoverLogos(){
    view.querySelectorAll('[data-cover-logo]').forEach(image=>image.addEventListener('error',()=>{
      ...
      slot.replaceWith(el(`<span class="${slot.className||''} cover-logo-empty no-print">${escapeHtml(image.dataset.coverLogo)}<small>Unggah di Data Referensi → Logo dan Tanda Tangan</small></span>`));
    },{once:true}));
  }
