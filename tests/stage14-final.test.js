...
test('identitas Tahap 14 konsisten pada web dan Android',()=>{assert.deepEqual([APP_VERSION,VERSION_CODE,APP_SCHEMA_VERSION],['1.1.0',3,3]);const gradle=read('android/app/build.gradle');assert.match(gradle,/applicationId "id\.sch\.sdn\.satriajaya01\.erapor"/);assert.match(gradle,/\?: '3'/);assert.match(gradle,/\?: '1\.1\.0'/);});

test('template siswa adalah XLSX biner nyata dan import tetap Preview sebelum Commit',()=>{setup();const template=studentTemplateWorkbook();assert.equal(String.fromCharCode(...new Uint8Array(template).slice(0,2)),'PK');assert.deepEqual(readWorkbookRows(template)[0],STUDENT_CSV_HEADERS);const bytes=createWorkbookBytes('Data Siswa',[STUDENT_CSV_HEADERS,['1401','001401','Siswa XLSX','P','Bekasi','2015-03-04','Orang Tua','','Alamat']]);const preview=previewStudentWorkbookImport(teacher,bytes,{classId:'5B'});assert.equal(preview.canCommit,true);assert.equal(listStudents(teacher).length,0);assert.equal(commitStudentImport(teacher,preview).length,1);assert.equal(listStudents(teacher)[0].name,'Siswa XLSX');});

test('template transkrip adalah XLSX nyata dan mengikuti data scope',()=>{setup();createStudent(teacher,studentInput());const bytes=transcriptTemplateWorkbook(teacher);assert.equal(String.fromCharCode(...new Uint8Array(bytes).slice(0,2)),'PK');const rows=readWorkbookRows(bytes);assert.deepEqual(rows[0],['NISN','NIS','Nama','Kode Mapel','Mata Pelajaran','Nilai']);assert.ok(rows.length>1);});
