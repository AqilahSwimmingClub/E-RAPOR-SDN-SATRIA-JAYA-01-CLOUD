...
test('identitas Tahap 14 konsisten pada web dan Android',()=>{assert.equal(APP_SCHEMA_VERSION,4);const gradle=read('android/app/build.gradle');assert.match(gradle,/applicationId "id\.sch\.sdn\.satriajaya01\.erapor"/);assert.match(gradle,new RegExp(`\\?: '${VERSION_CODE}'`));assert.match(gradle,new RegExp(`\\?: '${APP_VERSION.replace(/\./g,'\\.')}'`));});

test('template siswa adalah XLSX biner nyata dan import tetap Preview sebelum Commit',()=>{setup();const template=studentTemplateWorkbook();assert.equal(String.fromCharCode(...new Uint8Array(template).slice(0,2)),'PK');assert.deepEqual(readWorkbookRows(template)[0],STUDENT_CSV_HEADERS);const bytes=createWorkbookBytes('Data Siswa',[STUDENT_CSV_HEADERS,['1401','001401','Siswa XLSX','P','Islam','Bekasi, 4 Maret 2015','Orang Tua','','Alamat']]);const preview=previewStudentWorkbookImport(teacher,bytes,{classId:'5B'});assert.equal(preview.canCommit,true);assert.equal(listStudents(teacher).length,0);assert.equal(commitStudentImport(teacher,preview).length,1);assert.equal(listStudents(teacher)[0].name,'Siswa XLSX');});
...
test('status kenaikan dan kelulusan tidak pernah menahan kelengkapan rapor',()=>{setup();createStudent(teacher,studentInput());const ganjil=getReportCompleteness(teacher).students[0];assert.equal(Object.hasOwn(ganjil.categories,'finalStatus'),false);const genap={...teacher,semester:`Genap ${ACADEMIC_YEAR}`};createStudent(genap,studentInput({id:'genap',nis:'1402',nisn:'001402'}));assert.equal(Object.hasOwn(getReportCompleteness(genap).students[0].categories,'finalStatus'),false,'guru menentukan sendiri kapan mengisinya');});
