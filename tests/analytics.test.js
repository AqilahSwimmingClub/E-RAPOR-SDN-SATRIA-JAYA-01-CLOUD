...
test('Overall class completion uses active subjects and actual saved reports',()=>{
  useMemoryStorage();tugaskan(base,SUBJECTS_DEFAULT.map(subject=>subject.id));onlyAgama(base);const first=addStudent(base,1);const second=addStudent(base,2);saveManualReportScore(base,'agama',first.id,88);saveManualReportScore(base,'agama',second.id,76);description(base,first);
...
test('Report statistics calculate average, extrema, distribution, and per-subject graph values',()=>{
  useMemoryStorage();tugaskan(base,SUBJECTS_DEFAULT.map(subject=>subject.id));const first=addStudent(base,1);const second=addStudent(base,2);const third=addStudent(base,3);saveManualReportScore(base,'agama',first.id,60);saveManualReportScore(base,'agama',second.id,80);saveManualReportScore(base,'agama',third.id,100);saveManualReportScore(base,'mtk',first.id,90);
