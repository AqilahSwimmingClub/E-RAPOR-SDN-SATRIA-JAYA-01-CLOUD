...
test('identitas versi Tahap 14 memakai versionName 1.1.0, versionCode 3, dan schema 3',()=>{
  assert.equal(APP_VERSION,'1.1.0');assert.equal(VERSION_CODE,3);assert.equal(APP_SCHEMA_VERSION,3);
});
...
test('migration membuat safety snapshot lengkap beserta metadata versi',()=>{
  ...
  assert.equal(snapshots[0].fromAppVersion,'0.9.0');assert.equal(snapshots[0].toAppVersion,'1.1.0');assert.equal(snapshots[0].fromSchemaVersion,1);assert.equal(snapshots[0].toSchemaVersion,3);assert.equal(snapshots[0].status,'SUCCESS');assert.ok(snapshots[0].migratedAt);
});
