...
test('Weighted report formula returns raw and rounded values',()=>{
  useMemoryStorage();siapkanPenugasan(teacher5b,teacher5c,teacher5bGenap);const student=addStudent(teacher5b);scores(teacher5b,'agama',student,completeValues);
...
test('A blank component is never zero and never divides the report score',()=>{
  useMemoryStorage();siapkanPenugasan(teacher5b,teacher5c,teacher5bGenap);const student=addStudent(teacher5b);scores(teacher5b,'agama',student,{formative:80,daily:'',practice:90,scopeSummative:60,semesterSummative:100});
...
test('Report score uses weights normalized over filled components only',()=>{
  useMemoryStorage();siapkanPenugasan(teacher5b,teacher5c,teacher5bGenap);const student=addStudent(teacher5b);
...
test('Bobot mengubah nilai rapor dan KKTP tetap menentukan ketuntasan',()=>{
  useMemoryStorage();siapkanPenugasan(teacher5b,teacher5c,teacher5bGenap);const student=addStudent(teacher5b);const values={formative:100,daily:0,practice:0,scopeSummative:0,semesterSummative:0};scores(teacher5b,'agama',student,values);scores(teacher5b,'mtk',student,values);
...
test('Daily score switches between manual assessment and attendance conversion without changing attendance',()=>{
  useMemoryStorage();siapkanPenugasan(teacher5b,teacher5c,teacher5bGenap);const student=addStudent(teacher5b);saveAssessmentScores(teacher5b,'agama','daily',{[student.id]:40});saveAttendance(teacher5b,'2026-08-01',{[student.id]:'Hadir'});saveAttendance(teacher5b,'2026-08-02',{[student.id]:'Sakit'});const attendanceBefore=JSON.parse(JSON.stringify(loadDb().attendance));
...
test('Manual override stores the previous automatic score as reference',()=>{
  useMemoryStorage();siapkanPenugasan(teacher5b,teacher5c,teacher5bGenap);const student=addStudent(teacher5b);scores(teacher5b,'agama',student,completeValues);saveAutomaticReportScores(teacher5b,'agama');const overridden=saveManualReportScore(teacher5b,'agama',student.id,95);
...
test('Report scores remain isolated by class, subject, and semester',()=>{
  useMemoryStorage();siapkanPenugasan(teacher5b,teacher5c,teacher5bGenap);const sharedId='report-shared-student';const student5b=addStudent(teacher5b,1,sharedId);const student5c=addStudent(teacher5c,1,sharedId);const studentGenap=addStudent(teacher5bGenap,1,sharedId);
...
test('Description generation now uses Butir CP, not Learning Objectives',()=>{
  ...
  useMemoryStorage();siapkanPenugasan(teacher5b,teacher5c,teacher5bGenap);const student=addStudent(teacher5b);const {best,improve}=addTwoObjectives(teacher5b,'bindo');
...
test('Report import previews without committing and saves only after commit',()=>{
  useMemoryStorage();siapkanPenugasan(teacher5b,teacher5c,teacher5bGenap);const student=addStudent(teacher5b);const template=reportTemplateCsv(teacher5b,'agama');const csv=template.replace(`${student.nisn},agama,`,`${student.nisn},agama,89`);const preview=previewReportImport(teacher5b,'agama',csv);
