...
test('Data Referensi tidak membuat tahun, semester, rombel, atau mapel duplikat',()=>{
  ...
  createAcademicYear(admin,'2027/2028');assert.throws(()=>createAcademicYear(admin,'2027/2028'),/sudah tersedia/);refs=getReferenceOverview();assert.equal(refs.academicYears.filter(item=>item.id==='2027/2028').length,1);assert.deepEqual(listReferenceSemesters({academicYear:'2027/2028'}).map(item=>item.name),['Ganjil','Genap']);
